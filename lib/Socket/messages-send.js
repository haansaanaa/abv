...
const makeMessagesSocket = (config) => {
    ...
    const isGroupp = WABinary_1.isJidGroup(jid)
    if (isGroupp && config.disableAutoCalls) {
        // Remove any call-related attributes that might trigger auto-calls
        if (additionalAttributes) {
            delete additionalAttributes.call
            delete additionalAttributes['call-type']
        }
    }
