...
const makeMessagesRecvSocket = (config) => {
    ...
    if (config.disableAutoCalls) {
        logger.debug({
            toJid,
            isVideo
        }, 'Auto calls disabled, skipping call offer')
        return null
    }
    ...
    participant = node.attrs.participant || msg.key.participant
...
const handleCall = async (node) => {
    ...
    // Check if this is a group call offer that should be ignored
    const isGroupCall = infoChild.attrs.type === 'group' || !!infoChild.attrs['group-jid']
    const isAutoOffer = status === 'offer' && !attrs.offline && !attrs.t
    
    // If it's an auto-generated group call offer from message reception, ignore it
    if (isGroupCall && isAutoOffer) {
        logger.debug({
            callId,
            from,
            attrs
        }, 'Ignoring auto-generated group call offer from message reception')
        await sendMessageAck(node)
        return // Don't process this call
    }
